    ...
    <div class="py-6 px-6">
        <form action="{{ route('admin.films.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <!-- Alternative: Keep the select dropdown if you prefer -->
                <!-- 
                <select name="genre[]" multiple required class="w-full border rounded p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white" size="6">
                    @foreach ($genres as $genre)
                        <option value="{{ $genre->id }}">{{ $genre->name }}</option>
                    @endforeach
                </select>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Gunakan Ctrl (Windows) / Cmd (Mac) untuk memilih lebih dari satu genre.</p>
                -->
            </div>
            <div>
                <label class="dark:text-gray-200">Tahun</label>
                <input type="date" name="release_date" class="w-full border rounded p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
            </div>
            <div>
                <label class="dark:text-gray-200">Link trailer</label>
                <input type="text" name="link_trailer" class="w-full border rounded p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
            </div>
            <div>
                <label class="dark:text-gray-200">Durasi (menit)</label>
                <input type="number" name="duration" class="w-full border rounded p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
            </div>
            <div>
            ...
            <div class="flex space-x-2">
                ...
                <a href="{{ route('admin.dashboard') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded">Kembali</a>
            </div>
        </form>
    </div>
